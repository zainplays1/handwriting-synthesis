...
class ChunkSynthesizer(object):
    ...
    def __init__(self, hand=None, bias=0.9, style=9):
        if hand is None:
...
class CanvasStitcher(object):
    ...
    def stitch(self, instructions, synthesizer):
        ...
        for instruction in instructions:
            ...
            # model coordinates are y-up; SVG is y-down
            coords[:, 1] *= -1
    ...
    def _symbol_to_coords(self, instruction):
        ...
        hand_amp = max(0.35, size * 0.05)

        def n(scale=1.0):
            return self._jitter(0.0, amplitude=hand_amp * scale)

        if text == '+':
            return np.array([
                [x + n(), y + n(0.6), 0.0],
                [x + size + n(), y + n(0.6), 1.0],
                [x + 0.52 * size + n(0.5), y - 0.55 * size + n(), 0.0],
                [x + 0.48 * size + n(0.5), y + 0.52 * size + n(), 1.0],
            ])

        return np.array([
            [x + n(), y - 0.23 * size + n(0.6), 0.0],
            [x + size + n(), y - 0.18 * size + n(0.6), 1.0],
            [x + n(), y + 0.19 * size + n(0.6), 0.0],
            [x + size + n(), y + 0.24 * size + n(0.6), 1.0],
        ])
